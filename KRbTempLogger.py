...
class userInterface(QtGui.QWidget):
    def __init__(self):
        ...
        self.listener.connect(
            self.listener, QtCore.SIGNAL("dataReceived"), self.dataReceived
        )
    ...
    def setup(self):
        ...
        self.temperatureStatic.setAlignment(
    ...
    def dataReceived(self):
        ...
        request_url = "{}/api/v2/write?org={}&bucket={}&precision=s".format(
        ...
        try:
            response = requests.post(
                request_url, headers=request_headers, data=request_data
            )
        except Exception as e:
            print("Error sending data to influxdb: {}".format(e))
    ...
    def updateParameters(self):
        ...
        try:
            ...
            errEnc = True
            print(
                "Proportional gain must be set to an integer. Value reverted to default."
            )
        ...
        try:
            ...
            errEnc = True
            print(
                "Derivative gain must be set to an integer. Value reverted to default."
            )
        ...
        if not errEnc:
            self.firstRun = False
            transmit = "{0:.2E},{1:.2E},{2:.2E},{3:03d}".format(
                kp, ki, kd, temp2integer(DEFAULTSP)
            )
